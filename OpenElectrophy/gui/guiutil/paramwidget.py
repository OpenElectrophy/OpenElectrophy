...
class ParamWidget(QFrame) :
	...
	def __init__(self, params  , parent = None ,
		...
		if title is not None:
			v1.addWidget(QLabel('<b>'+title+'</b>'))
		
		if self.keyformemory is not None and applicationdict is not None:
		...
		for i in range(len(self.list_name)) :
			...
			if self.params[name].has_key('password') :
				if self.params[name]['password']:
		...
		if key in self.params.keys() :
			...
			if self.params[key]['widgettype'] ==  QCheckBox:
				...
				if val in self.params[key]['possible'] :
					...
					items = [ unicode(v) for v in self.params[key]['possible'] ]
					wid.clear()
					wid.addItems(items)
	...
	def __getitem__(self,key):
		if key in self.params.keys() :
			...
			if self.params[key]['widgettype'] ==  QCheckBox:
				...
				return self.params[key]['type'](wid.toPlainText())
...
class ParamDialog(QDialog):
	...
	def get_dict(self):
		return self.param_widget.get_dict()
